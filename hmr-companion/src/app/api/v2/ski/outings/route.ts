...
export async function GET(req: Request) {
  ...
  if (routeId) {
    ...
    if (route.activity !== "ski") {
      return NextResponse.json({ error: "Il percorso non è di scialpinismo" }, { status: 400 });
    }
    const canReadRoute =
    ...
    return NextResponse.json({ route_id: routeId, outings: serializeSkiOutings(rows) });
  }

  if (scope === "mine") {
    const rows = listOutingsVisibleForUser(auth.email);
    return NextResponse.json({ scope: "mine", outings: serializeSkiOutings(rows) });
  }
...
export async function POST(req: Request) {
  ...
  if (body.make_route_public !== false && route.visibility !== "public") {
    updateUserRoute(routeId, { visibility: "public" });
  }
  ...
  insertSkiOuting({
    ...
    snow_notes: (body.snow_notes ?? "").trim(),
    created_at: now,
    updated_at: now,
  });
