...
export function elevationGainLossSmoothed(
  ...
  for (let i = 1; i < smoothed.length; i++) {
    ...
    if (dir >= 0) {
      if (e > candidate) {
        ...
        if (candidate > lastExtreme) gain += candidate - lastExtreme;
    ...
    if (dir <= 0) {
      if (e < candidate) {
        ...
        if (candidate < lastExtreme) loss += lastExtreme - candidate;
        lastExtreme = candidate;
        dir = 1;
        candidate = e;
      }
  ...
  if (candidate > lastExtreme) gain += candidate - lastExtreme;
  else if (candidate < lastExtreme) loss += lastExtreme - candidate;
  return { gain, loss };
}
